...
async fn store_book_cover_is_sniffed_not_trusted_from_content_type() {
    ...
    let content_type = app
        .storage_object_content_type(&app.covers_bucket, cover_id)
        .await;
...
async fn store_book_cover_with_unknown_format_returns_415() {
    ...
    let obj_count = app.storage_objects_count(&app.covers_bucket).await;
    let covers = app.db_fetch_covers(book_id).await;
...
async fn store_book_cover_under_the_limit_passes() {
    ...
    let obj_exists = app
        .storage_object_exists(&app.covers_bucket, cover_id)
        .await;
    assert!(obj_exists);
}
...
async fn get_book_covers_lists_uploads_in_upload_order() {
    ...
    let first_id = app.fixture_insert_cover(book_id, COVER_PNG).await;
    let second_id = app.fixture_insert_cover(book_id, COVER_JPG).await;
    let third_id = app.fixture_insert_cover(book_id, COVER_WEBP).await;
...
async fn delete_book_cover_removes_it_from_the_gallery_and_purges_the_object() {
    ...
    let first_obj_exists = app
        .storage_object_exists(&app.covers_bucket, first_id)
        .await;
    let second_obj_exists = app
        .storage_object_exists(&app.covers_bucket, second_id)
        .await;
