...
use crate::{
    entity::{Book, BookDetails, BookWrite, Pagination},
    error::ServiceError,
    service::Service,
};

impl Service {
    pub async fn store_book(&self, item: BookWrite) -> Result<(), ServiceError> {
        self.database.store_book(item).await.map_err(Into::into)
    }

    pub async fn get_book(&self, id: Uuid) -> Result<BookDetails, ServiceError> {
    ...
    pub async fn update_book(&self, item: BookWrite) -> Result<(), ServiceError> {
