create table if not exists book_titles (
	...
	constraint fk_book_titles_books_book_id foreign key(book_id) references books(id),
	constraint fk_book_titles_languages_language_id foreign key(language_id) references languages(id),
	constraint check_length_book_titles_name check(char_length(name) <= 255)
);

create index if not exists idx_book_titles_book_id on book_titles(book_id);
