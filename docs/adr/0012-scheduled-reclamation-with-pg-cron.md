...
**pg_cron exists in production only, installed as an OS package.** Production Postgres is a standalone host install, so there is no compose file to overlay. It follows a runbook (`deploy/postgres/README.md`): install `postgresql-18-cron`, set `shared_preload_libraries = 'pg_cron'` and `cron.database_name = 'manga_theka'` (the default is `postgres`, and a wrong value fails silently by scheduling against the wrong database), restart, `create extension pg_cron`, then apply `bootstrap.sql` as a superuser. Dev and CI run stock `postgres:18` with no extension package at all. *Rejected: a repo-built image carrying the package dormant into dev and CI. It buys parity with an extension that is never preloaded, never created and never called there, and pays for it with an image every developer and every CI run has to build -- and, in CI, with a Postgres started by hand, because a service container cannot be built from the checkout.*

The schedule is therefore the one part of Reclamation the suite cannot cover, which is why **every schedule change is tested by calling the function directly before it is scheduled** -- `bootstrap.sql` only registers what has already been shown to work, and its runbook step is not optional.

**The jobs run as a role that holds nothing else.** `bootstrap.sql` needs a superuser to `create extension`, but pg_cron records the *scheduling* role as the run-as role, and both function bodies are owned by the role that runs the migrations -- the application's. Scheduling as superuser would therefore turn the application's database credentials into superuser execution within the hour, by replacing a body. The jobs are scheduled as `reclamation` instead, a role granted only the reads and deletes the two sweeps perform; both functions also pin `search_path` (with `pg_temp` last, so a temp relation cannot shadow a table) and have `execute` revoked from `public`.

That role has to carry `LOGIN`, because pg_cron checks `rolcanlogin` when a job is scheduled and refuses a `nologin` one outright. It is an attribute rather than an opening only because `cron.use_background_workers = on` is a prerequisite: pg_cron then runs jobs in a background worker instead of opening a libpq connection, so the role needs no password and no `pg_hba.conf` entry. With the default `off`, the runbook would instead have to admit `reclamation` in `pg_hba.conf`, and the remedy reached for first is a localhost `trust` line -- which lets any local process authenticate as any role, and is a worse outcome than the superuser schedule this whole arrangement exists to avoid. `bootstrap.sql` therefore refuses to apply unless background workers are on.
...
Holding that invariant under concurrency decides the shape of both statements: the enqueue reads the delete's `returning`, not the candidate set the delete was aimed at. The two differ, because a row picked under the statement's snapshot can be committed to by someone else before the delete reaches it -- an uploader committing a page in the same second the hourly sweep runs. Each delete therefore **restates its own predicate** (`sort_order is null` on the page sweep, `visibility` and `updated_at` on the book purge), which read-committed re-evaluates against the row version it waited for, so a page committed mid-sweep survives it. Enqueueing from the candidate set instead would record a key for a row that was never deleted, and its image would be dropped from storage while it was still on screen.
...
Each scheduled job arms `statement_timeout` before calling its function -- 30 s for the page sweep, 120 s for the book purge. It cannot live in the function body as first designed: `set local statement_timeout` inside a function does not re-arm the timer for the call already in progress (verified on 18.6), so it would bound only whatever ran after the function returned. `bootstrap.sql` therefore sets it in the job command, where it does bound the run. `pg_cron` will not start a run of a job while its previous run is still active, so a slow run delays the next rather than overlapping it. A third scheduled job prunes `cron.job_run_details` older than thirty days -- the scheduler's own log is otherwise the one table nothing reclaims.
