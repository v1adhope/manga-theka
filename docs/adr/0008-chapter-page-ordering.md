...
`chapter_pages.sort_order` is contiguous from 1 within a release, `(release_id, sort_order)` unique. ADR-0007 dropped ordering from the covers gallery precisely because a value meaning something only relative to its siblings turns every single-row write into a statement about all of them; pages genuinely need order, so that bill comes due here. What follows is its shape, and three PostgreSQL behaviours that make the obvious implementations wrong -- each established against `postgres:17` rather than reasoned about, since each contradicts a reasonable expectation.

**A plain unique index cannot survive a positional shift.** Reordering moves rows onto positions their siblings still hold, and PostgreSQL checks uniqueness per row *during* the statement, not at its end:
...
**`CHECK` constraints cannot be marked `DEFERRABLE`**, eliminating the standard workaround. The usual way to shift without deferral is a two-phase negative-offset pass -- negate affected rows, then negate-and-increment them back -- requiring an intermediate state where `sort_order` is negative. With `check(sort_order between 1 and 200)` in place, the first statement is rejected outright and can't be deferred through. Keeping the plain index would have meant dropping the floor guarantee that `Sort Order` starts at 1, which `CONTEXT.md` states as part of the term's meaning.
