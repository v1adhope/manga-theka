...
Wired: `POST /books` -> any signed-in role; content-write routes (`creators`, `chapters`, `releases`) -> `[Uploader, Moderator, Admin]`; `Book`/cover write routes (`PUT /books/{id}`, `POST /books/{id}/covers`, `PUT /books/{id}/main-cover`, `DELETE /covers/{id}`) -> any signed-in role at the route layer only, with the real gate -- visibility-keyed, not a flat role set -- in `BookAccess::ensure_record_writable` (ADR-0009); `DELETE` of a creator or book -> `[Moderator, Admin]`; `POST /feedbacks` anonymous, other `feedbacks` routes `[Moderator, Admin]`. `PUT /books/{id}/visibility` and `GET /books?visibility=` gates: ADR-0009.
